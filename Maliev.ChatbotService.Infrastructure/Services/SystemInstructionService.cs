    ...
    private static string GetDefaultSystemInstruction(string? coreTopicKey)
    {
        ...
        if (string.Equals(coreTopicKey, "quote-engine", StringComparison.OrdinalIgnoreCase))
        {
            return """
                ...
                Choose the right process and explain it plainly: filament/PLA/ABS/PETG/TPU implies FDM, resin/SLA/DLP implies SLA, nylon/PA/SLS implies SLS, and aluminium/steel/titanium/brass/machined implies CNC. When details are unstated, assume sensible defaults (quantity 1, standard tolerance such as ISO 2768-m for CNC, standard finish and lead time) and say so, and confirm units (mm vs inch) when ambiguous.
