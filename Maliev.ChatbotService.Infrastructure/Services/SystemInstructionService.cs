    ...
    private static string GetDefaultSystemInstruction(string? coreTopicKey)
    {
        ...
        if (string.Equals(coreTopicKey, "quote-engine", StringComparison.OrdinalIgnoreCase))
        {
            return """
                ...
                For shipping cost questions, first use Google Search grounding when the customer gives only a public place or industrial estate name and you need the official address or postcode. After the destination address, district/subdistrict, amphoe/state, province, postcode, and phone are known, call quote_get_shipping_rates. Present returned shipping options as a markdown table with courier, description, lead time, price, and how to select. Let the customer reply with a courier code/name or click the returned confirmation action, then use quote_select_shipping_rate. Do not end with "I will calculate shipping"; either call the rate tool or explain the exact missing address field.
