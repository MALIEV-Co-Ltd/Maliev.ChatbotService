...
public class QuotationToolHandler(IHttpClientFactory httpClientFactory) : BaseToolHandler(httpClientFactory)
{
    ...
    public override async Task<string> ExecuteAsync(string toolName, Dictionary<string, object> args, string? userToken, CancellationToken cancellationToken)
    {
        ...
        {
            "search_quotations" => await SearchQuotations(args, userToken, cancellationToken),
            "get_quotation" => await GetAsync($"/quotation/v1/quotations/{GetStringArg(args, "quotation_id")}", userToken, cancellationToken),
            _ => """{"error": "Unknown quotation tool"}"""
        };
    }

    private async Task<string> SearchQuotations(Dictionary<string, object> args, string? userToken, CancellationToken ct)
    {
