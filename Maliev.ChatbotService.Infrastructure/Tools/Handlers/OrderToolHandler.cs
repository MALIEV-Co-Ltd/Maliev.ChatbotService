...
public class OrderToolHandler(IHttpClientFactory httpClientFactory) : BaseToolHandler(httpClientFactory)
{
    ...
    public override async Task<string> ExecuteAsync(string toolName, Dictionary<string, object> args, string? userToken, CancellationToken cancellationToken)
    {
        ...
        {
            "search_orders" => await SearchOrders(args, userToken, cancellationToken),
            "get_order" => await GetAsync($"/order/v1/orders/{GetStringArg(args, "order_id")}", userToken, cancellationToken),
            _ => """{"error": "Unknown order tool"}"""
        };
    }

    private async Task<string> SearchOrders(Dictionary<string, object> args, string? userToken, CancellationToken ct)
    {
