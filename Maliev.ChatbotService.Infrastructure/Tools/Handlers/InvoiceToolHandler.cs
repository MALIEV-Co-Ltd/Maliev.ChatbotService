...
public class InvoiceToolHandler(IHttpClientFactory httpClientFactory) : BaseToolHandler(httpClientFactory)
{
    ...
    public override async Task<string> ExecuteAsync(string toolName, Dictionary<string, object> args, CancellationToken cancellationToken)
    {
        ...
        {
            "search_invoices" => await SearchInvoices(args, cancellationToken),
            "get_invoice" => await GetAsync($"/invoice/v1/invoices/{GetStringArg(args, "invoice_id")}", cancellationToken),
            _ => """{"error": "Unknown invoice tool"}"""
        };
    }

    private async Task<string> SearchInvoices(Dictionary<string, object> args, CancellationToken ct)
    {
